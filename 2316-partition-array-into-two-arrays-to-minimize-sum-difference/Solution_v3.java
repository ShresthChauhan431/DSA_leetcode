class Solution {
    ...
    public int minimumDifference(int[] nums) {
        ...
        for(Map.Entry<Integer, Set<Long>> entry: map2.entrySet()){
            List<Long> l = new ArrayList<>(entry.getValue());
            l.sort(null);
            map.put(entry.getKey(), new ArrayList<>(l));
        }
        ...
        for(Map.Entry<Integer, Set<Long>> entry: map1.entrySet()) {
            ...
            List<Long> list1 = new ArrayList<>(entry.getValue());

            for(long a: list1) {
                double b = sum / 2.0 - a;

                List<Long> list2 = map.get(needlen);
                int x = lowerBound(b, list2);

                if(x < list2.size()) {
                    long sum1 = a + list2.get(x);
                    long sum2 = sum - sum1;
                    ans = Math.min(ans, Math.abs(sum1 - sum2));
                }
                if(x > 0){
                    long sum1 = a + list2.get(x - 1);
                    long sum2 = sum - sum1;
                    ans = Math.min(ans, Math.abs(sum1 - sum2));
                }
            }
    ...
    public int lowerBound(double tar,  List<Long> list){
        // list.sort(null);
        int i = 0;
        int j = list.size(); 
        while(i < j){
